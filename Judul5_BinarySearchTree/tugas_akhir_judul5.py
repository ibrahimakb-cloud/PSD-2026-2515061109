...
class BST:
    ...
    def _insert(self, root, judul, rating, penonton):
        if root is None:
            return Node(judul, rating, penonton)
    ...
    def tampilkan_inorder(self, root):
        if root is not None:
            ...
            print(f"- {root.judul} | Rating: {root.rating} | Penonton: {root.penonton}")
...
def main():
    ...
    bst.insert("gurren lagann", 9.9, 2792639)
    bst.insert("petualangan aneh jojo", 9.3, 5058902)
    bst.insert("Mob Psycho 100", 9.7, 3040058)
